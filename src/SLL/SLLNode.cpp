...
void SLLNode::drawCur(){
    DrawRectangleRounded(node, 100, 100, (isHighlight)? nodeHighlightColor : nodeColor);
    DrawText(std::to_string(this->value).c_str(),position.x+SLLNodeSize.x/2.0f-MeasureText(std::to_string(this->value).c_str(),SLLNodeFontSize)/2.0f,position.y+SLLNodeSize.y/2.0f-SLLNodeFontSize/2.0f,SLLNodeFontSize,(isHighlight)? nodeHighlightTextColor : nodeTextColor);
    if (Pointer) {
        Pointer->draw();
    }
}
